                                        ...
                                        lxb_tag_id_t ex_tag, lxb_ns_id_t ex_ns)
{
    ...
    while (lexbor_array_length(tree->open_elements) != 0) {
        ...
        if (lxb_html_tree_node_is_menuitem(node)
            && !(node->local_name == ex_tag && node->ns == ex_ns))
        {
    ...
    while (idx != 0) {
        ...
        switch (node->local_name) {
            case LXB_TAG_SELECT: {
                ...
                while (ancestor != 0) {
                    lxb_dom_node_t *parent = list[--ancestor];
                    if (lxb_html_tree_node_is(parent, LXB_TAG_TEMPLATE)) {
                        break;
                    }
                    if (lxb_html_tree_node_is(parent, LXB_TAG_TABLE)) {
