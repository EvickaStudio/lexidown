...
lxb_html_tree_select_in_scope(lxb_html_tree_t *tree)
{
    size_t idx = tree->open_elements->length;
    lxb_dom_node_t *node;

    while (idx != 0) {
        node = tree->open_elements->list[--idx];
        if (lxb_html_tree_node_is(node, LXB_TAG_SELECT)) {
            return node;
        }
        if (!lxb_html_tree_node_is(node, LXB_TAG_OPTION)
            && !lxb_html_tree_node_is(node, LXB_TAG_OPTGROUP))
        {
            return NULL;
        }
    }
                                                     ...
                                                     lxb_html_token_t *token)
{
    ...
    node = lxb_html_tree_select_in_scope(tree);
    if (node == NULL) {
