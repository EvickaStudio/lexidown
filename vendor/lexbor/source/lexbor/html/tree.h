/* Modified for Turndown/Domino compatibility; see vendor/lexbor/README.md. */
/*
...
/* Domino retains the obsolete menuitem element as an optional-end-tag element. */
lxb_inline bool
lxb_html_tree_tag_is_menuitem(lxb_tag_id_t tag_id)
{
    size_t length;
    ...
    name = lxb_tag_name_by_id(tag_id, &length);
    return length == 8 && memcmp(name, "menuitem", 8) == 0;
}

lxb_inline bool
lxb_html_tree_node_is_menuitem(const lxb_dom_node_t *node)
{
    return node != NULL && node->ns == LXB_NS_HTML
        && lxb_html_tree_tag_is_menuitem(node->local_name);
}
